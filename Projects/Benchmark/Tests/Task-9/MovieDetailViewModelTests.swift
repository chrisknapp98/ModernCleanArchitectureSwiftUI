...
class MovieDetailViewModelTests: XCTestCase {
    ...
    func test_givenMovie_whenTapOnMovie_thenCallCoordinatorOnce() {
        // given
        movieMock = mockMovie(title: "New Movie")
        let sut = sut
        ...
        XCTAssertEqual(moviesCoordinatorMock.showAddMovieToCustomListCallCount, 1)
        XCTAssertEqual(moviesCoordinatorMock.showMovieDetailForMovie?.title, movieMock.title)
    }
}
...
class MoviesCoordinatorMock: MoviesCoordinator {
    ...
    var showMovieDetailForMovie: Movie?
