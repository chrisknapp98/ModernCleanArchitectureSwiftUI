...
final class MoviesViewModelTests: XCTestCase {
    ...
    func test_givenPopularMovies_whenFilteringMoviesToDiscover_thenFindMovies() async {
        // given
        let expectedMovies = [
        ...
        let sut = sut
        let filterCriteria: DiscoverMoviesRequest = .popular
        let fetchExpectation = expectation(description: "Fetch movies")
        discoverMoviesUseCase.onFetch = {
            fetchExpectation.fulfill()
        }
    
        // when
        sut.filter(request: filterCriteria)
        await fulfillment(of: [fetchExpectation], timeout: 1.0)
        
        // then
        XCTAssertEqual(sut.request, filterCriteria)
    ...
    func test_givenTopRatedMovies_whenFilteringMoviesToDiscover_thenFindMovies() async {
        // given
        let expectedMovies = [
            mockMovie(title: "Test Movie 1"),
            mockMovie(title: "Test Movie 2")
        ...
        discoverMoviesUseCase.results = expectedMovies
        let sut = sut
        let filterCriteria: DiscoverMoviesRequest = .topRated
        let fetchExpectation = expectation(description: "Fetch movies")
        discoverMoviesUseCase.onFetch = {
            fetchExpectation.fulfill()
        }
